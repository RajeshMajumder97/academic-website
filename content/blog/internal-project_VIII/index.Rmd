...
```{r,echo=F}
knitr::kable(head(survival::lung[,c(2,3,4)]),'html')
```
...
library(KMsurv)
data(alloauto)
knitr::kable(head(alloauto),'html')
```
...
```
```{r,echo=FALSE}
SurvPlot=function(time,name=NULL,status,C=NULL,E=NULL,title=NULL,bar.color="#808080",C.color="#FF6666",E.color="#66B2FF",text.cex=0.8,text.adjs=0.5,point.cex=0.8,legend.posi=locator(1),legend.cex=0.8){
  
  stopifnot("'time' is missing or may be it is not a 'numaric'"= class(time)=="numeric",
            "'status' is missing or may be it is not a 'factor'"= class(status)=="factor")
  
  if(is.null(title)!=T){
    
    stopifnot("'title' must be a character"=class(title)=="character")
  }
  
  if(is.null(name)==T){
    name=as.character(1:length(time))
  }
  ...
  if(is.null(C)==T | is.null(E)==T){
    status=factor(status,levels = levels(status),labels = c("C","E"))  
  }
  else{
    status=factor(status,levels = c(C,E),labels = c("C","E"))
  }
  ...
  barplot(time,names=name,horiz = T,space = 2,xlim = c(min(time,na.rm = T)+(-1),max(time,na.rm = T)+1),col = bar.color,main = title,xlab = "Time",ylab = "Subject")
  text(rep(0+(-1)+text.adjs,length(time)),point,round(time,1),cex=text.cex,font = 4)
  for(i in 1:length(status)){
    if(status[i]=="C"){
      points(time[i],point[i],pch=19,cex=point.cex,col=C.color) 
    }
    else{
      points(time[i],point[i],pch=17,cex=point.cex,col=E.color)
    }
  }
  abline(v=max(time),lty=2)
  box()
  legend(legend.posi,title = "Survival",legend = c("Censor","Event"),col=c(C.color,E.color),pch=c(19,17),cex = legend.cex)
}
...
```{r,echo=FALSE}
knitr::kable(head(survival::lung),'html')
```
...
```
```{r,echo=FALSE}
knitr::kable(head(MASS::Melanoma),'html')
```
